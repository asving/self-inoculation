"""Figures for the blog's results section (history-heavy arm, runs_h).
 1. mixed_shared_circuit.png : (left) HMM1-belief decoders fit in one situation, scored in the other;
...
for s in ('s0', 's1', 's2'):
    m = load(f'{RUNS}/{s}/anchored/model.pt')
    ...
    for name, key in zip(CAPS, CAP_IDX):
        ...
        for fit in 'AB':
            ...
            for test in 'AB':
                ...
                if test == 'A': Pt, Pd = P.flip_dist(Pt), P.flip_dist(Pd)   # finetuned net performs the swapped task in A
...
fig.suptitle("after mixed finetuning: cross-situation probing and steering of the HMM1 tracker (3 seeds, mean ± sd)")
fig.tight_layout(); fig.savefig(f'{OUT}/mixed_shared_circuit.png', dpi=130); print("saved mixed_shared_circuit.png")
json.dump(dict(dec=dec, steer=steer), open(f'{OUT}/mixed_shared_circuit.json', 'w'))
